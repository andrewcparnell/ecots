...
The `airquality` data set is included with R. You can get at it simply by typing `airquality` at the R command prompt. Let's suppose we're interested in predicting the variable `Ozone` from the other variables
...
1. Fit a linear regression model using `lm` with `Ozone` as the response and the other variables as covariates. Use the `summary` method to interpret your findings. (Note that R here is automatically removing rows with missing variables)
...
1. Fit a binomial glm (a logistic regression) with the binary variable as the repsonse and `width` as a covariate. Summarise your findings

1. Create a plot of the fitted values on top of a scatter plot of the data (hint: width on x-axis, binary reponse variable on y-axis)
...
2. The whitefly data set. This is binomial (as used in the lectures) but you might like to additionaly try some of the other variables and see which are important and why. The data set has particular issues with zero-inflation. See if you can find which zero data points are poorly predicted by the model.