...
- They also don't have to to be in 1 dimension. We can simulate random walks in 2, 3, ... dimensions by adding multivariate steps
...
- We repeate the autocorrelation calculation for 1 lag, 2, lags, 3 lags, etc
...
```{r simulate ARp, echo=TRUE, fig.height = 3}
set.seed(100)
T = 500
t_seq = 1:T
sigma = alpha = 1
...
- We can create a _prediction interval_ by also including the values $\hat{\sigma}$
