...
- This is very computationally intensive as we have to re-fit the model and get new parameter estimates at every step
...
lines(my_model_ARIMAX111$fitted, col = 'red')
```

## Accuracy measures from `forecast`

- For the fitted values you can also measures of accuracy such as root mean square error
...
## Implementing loo-CV for an ARIMAX model

```{r}
n_min = 30 # minimum length for model
n = nrow(wheat)
mae = forecasts = rep(NA, n - n_min)
# Loop through time series
for(i in 1:(n-n_min)) {
  # Fit to the training set
  curr_model = auto.arima(wheat$wheat[1:(i + n_min - 1)])
  # Create 1 step ahead forecasts
  forecasts[i] = forecast(curr_model, h = 1)[['mean']]
  # Get mean absolute error
  mae[i] = abs(forecasts[i] - wheat$wheat[i + n_min])
}
mean(mae)
```
...
```{r}
plot(wheat$wheat[31:n], type = 'l')
lines(forecasts, col = 'red')
```

- In fact all the ARIMA models chosen here are (0, 1, 0)
