...
plot(ff$year, exp(h_post), type = 'l')
```
...
with(geese, plot(julianday[o], scale(d13CPl[o])[,1],
                                  ylab = 'd13C',
                 xlab = 'Day of year'))
```
...
- We would like the model to be seasonal with period 365
...
- We have to separate out the model into two layers:
    ...
    1. The underlying frequency model defined at each time point
    
- A possible model:
$$y_t \sim N(\mu_{\mbox{day}_t}, \sigma_{\mbox{day}}^2)$$
$$\mu_{\mbox{day}} = \alpha + \beta \sin \left[ \frac{2 \pi \mbox{day}_t}{365}\right]$$

    
## JAGS code for a repeated measures frequency model

```{r}
stan_code = '
data {
  int<lower=0> N; # number of observations
  int<lower=0> N_day; # number of unique days
  vector[N] y; # response variable
  int day[N]; # variable to match days to observations
  vector[N] julianday; # variable to match days to observations
}
parameters {
  real alpha; # intercept
  real beta; # fourier parameter  
  real<lower=0> sigma; # st dev within day
}
transformed parameters{
  vector[N_day] mu; # repeated measure parameter
  for(t in 1:N_day) {
    mu[t] = alpha + beta * sin(2 * pi() * julianday[t] / 365);
  }
}
model {
  for (i in 1:N)
...
day = as.numeric(as.factor(geese$julianday))
stan_run_rm = sampling(stan_mod_rm,
                       data = list(y = scale(geese$d13CPl)[,1],
                                   N = nrow(geese),
                                   julianday = geese$julianday,
                                   day = day,
                                   N_day = length(unique(days))))
print(stan_run_rm)
```
...
```{r}
plot(stan_run_rm, pars = c('alpha', 'beta', 'sigma'))
```

# Plot the best fit model

```{r}
beta_post = summary(stan_run_rm, pars = c("beta"))$summary[,'50%']
alpha_post = summary(stan_run_rm, pars = c("alpha"))$summary[,'50%']
with(geese, plot(julianday[o], scale(d13CPl[o])[,1],
                                  ylab = 'd13C',
                 xlab = 'Day of year'))
x = 1:365
lines(x, alpha_post + beta_post * sin(2 * pi * x / 365), col = 'red')
mu_post = summary(stan_run_rm, pars = c("mu"))$summary[,'50%']
points(geese$julianday, mu_post[day], pch = 19)
```
...
- You could fit lots of different models with combinations of ecah of the variables
