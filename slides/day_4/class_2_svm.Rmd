...
model
{
  ...
  h[1] <- mu
  for(t in 2:T) {
    h[t] ~ dnorm(mu + phi * (h[t-1] - mu), sigma^-2)
  }
...
```{r, echo = FALSE}
plot(1918:1988, exp(ff_run_3$BUGSoutput$median$h), type = 'l', ylab = 'h')
h_post = ff_run_3$BUGSoutput$sims.list$h
upp = apply(h_post, 2, quantile, 0.75)
low = apply(h_post, 2, quantile, 0.25)
lines(1918:1988, exp(low), col = 'red', lty = 2)
lines(1918:1988, exp(upp), col = 'red', lty = 2)
h_med = exp(apply(ff_run_3$BUGSoutput$sims.list$h,2,'quantile',0.5)/2)
```
...
Even better again, despite many extra parameters due to $h$!
