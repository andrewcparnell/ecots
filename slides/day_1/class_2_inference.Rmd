...
- A probability distribution can come in (at least) three different forms:
    ...
    1. As an equation, which enables us to calculate
...
- This distribution is known as continuous which means that the values of $x$ can be real numbers
...
- Binomial. Discrete. Useful when the values have a lower and upper limit. Has two parameters $N$ and $p$ which determine how the values behave. Mean is $N \times p$ and standard deviation $\sqrt{N \times p \times (1-p)}$. R function is `dbinom`
...
```{r}
x = c(-0.23, -0.97, -1.94, -2.08, 2.55, 0.8, -0.03, -0.79, -0.6, 
1.75)
```
...
- Likelihood values tend to get very small very quickly, so most people instead calculate the log of the likelihood instead, which is also easier to calculate
...
- We would also like to know whether the probabiliy distribution we chose actually matches the data
- One neat way is a QQ-plot (Quantile-Quantile plo) which compares the quantiles of the data with the expected quantiles of the fitted probability distribution
...
- We only have a limited set of data so we can't possibly know them exactly. We need to have estimates of uncertainty about the parameters
...
- So if we find that a parameter has an estimated value of 6 with standard deviation 1.5, we can calculate the 95\% confidence intervals as between 3 and 9
...
- Later in the course we will meet a method (called MCMC) which finds the best values and gets the uncertainty in one go
